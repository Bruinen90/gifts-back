...
module.exports = {
  ...
  setReserved: async ({ reservation }, req) => {
    ...
    if (!userId) {
      throwAuthError('You need to be logged in to set "I will buy it status"');
    }
    try {
      ...
      if (reserved) {
        ...
        if (desiredWish.buyer !== userId) {
          throwAuthError("You can't cancel reservation of another user");
        }
