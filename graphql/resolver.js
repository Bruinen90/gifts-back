...
module.exports = {
  createUser: async ({ userInput }, req) => {
    ...
    if (!validator.isLength(userInput.username, { min: 3, max: 30 })) {
      errors.push({
        message: "Username should be at least 3 and maximum 30 characters long"
      });
    }
    if (errors.length > 0) {
  ...
  userDraws: async (_, req) => {
    ...
    try {
      const allUserDraws = await Draw.find({ creator: req.userId })
      ...
      console.log("Internal Errrr", err);
      // console.log(err);
    }
  },
  ...
  deleteDraw: async ({ drawId }, req) => {
    try {
      const requestedDraw = await Draw.findByIdAndDelete(drawId);
      if (requestedDraw.creator !== req.userId) {
        // Undo delete if not authenticated
        await requestedDraw.save();
        throwAuthError();
      }
      return { success: true };
    } catch (err) {
